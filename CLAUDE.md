...
- **Proactive anomaly check-ins**: same scheduler tick also runs `AnomalyCheckinService.maybe_probe` for users with `schedule_prefs.checkins_enabled`. Detection is heuristic ([app/services/anomaly_detector.py](app/services/anomaly_detector.py)) — no AI call, no statistical baselines yet (re-evaluate if false-positive rate becomes a real problem). Messages are templated via i18n (`checkin.*` keys) so they're deterministic and free; the service does not call Anthropic. Gating in [app/services/anomaly_checkin_service.py](app/services/anomaly_checkin_service.py): opt-in flag → quiet hours (08:00–22:00 user tz) → 24 h cooldown → daily-summary-already-fired-today → detector. Stamp before send so a Telegram failure doesn't loop.
