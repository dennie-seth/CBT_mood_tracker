...
RU: dict[str, str] = {
    ...
    "log.enter_sleep_hours": "Сколько часов ты спал? (например, 7.5)",
    ...
    "done.pick": "Какой план ты выполнил?",
    "done.ask_actual": "Насколько это реально подняло настроение? (1–10)",
    "done.saved_with_pred": "Готово — прогноз +{predicted}, факт +{actual}. Молодец.",
    "done.saved": "Готово — факт +{actual}. Молодец.",
    ...
    "help.text": (
        ...
        "Сразу после пробуждения, чтобы записать сколько ты спал.\n"
        ...
        "мысль → искажение → переформулировка). Когда поймал сильную "
        "негативную мысль и хочешь её разобрать.\n"
        "/backfill <дата> <метрика> <значение> — запись задним числом. "
        "Если забыл записать вчера или хочешь добавить старую запись.\n\n"
